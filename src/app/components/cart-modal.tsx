...
const CartModal = ({ isOpen, onClose }: any) => {
  ...
  return (
    <>
      <div
        className="fixed hidden lg:block inset-0 z-40 bg-black/20"
        onClick={onClose}
      />

      <div className="fixed hidden lg:block top-18 right-12 z-50 w-[630px] bg-white rounded-lg border shadow-lg animate-in slide-in-from-right-2 duration-300">
...
export const MobileCartModal = ({ isOpen, onClose }: any) => {
  if (!isOpen) return null;
  return (
    <>
      <Drawer open={isOpen} onOpenChange={onClose}>
        <DrawerContent showOverlay={false} className="p-6 space-y-6">
          <DrawerContent>
            <div className="border-b border-[#EEEEEE] pb-4 p-6">
              <h2 className="font-bricolageG font-semibold text-[32px]">
                My Cart
              </h2>
            </div>
            <div className="w-full border border-[#EEEEEE] rounded-lg p-4 flex  flex-col items-left justify-between gap-4">
              <div className="border border-[#F1BC9F] rounded-lg p-2.5 w-fit">
                <Image
                  src={"/take-away_1820160 1.png"}
                  alt="Take Away"
              ...
              <Badge className="bg-[#DBF4DE] px-2.5 py-1 h-fit rounded-xl text-[11px] text-[#2A8449] inline-flex ">
                Subscription
              </Badge>
              <p className="border-b border-[#4B4B4B] text-sm">
                View Subscription
              ...
              <div className="border-l border-[#EEEEEE] px-4 flex items-center gap-6">
                <div className="flex flex-col items-center gap-2">
                  <Image
                    ...
                    height={24}
                  />
                  <p className="text-sm tracking-wider text-[#878787]">
                    Cancel
                  </p>
                </div>
                <div className="flex flex-col items-center gap-2">
